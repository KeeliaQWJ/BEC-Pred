...
@click.option("--cols", nargs=3, type=str)
...
def main(input_filepath, output_filepath, cols, sep, length, radius):
    ...
    df = pd.read_csv(input_filepath, sep='\t', usecols=list(cols))
    ...
    for s in splits:
        ...
        df_split = df[df['split'] == s]

        print(f"{len(df_split)} reactions...")

        smiles = df_split['rxn'].to_numpy()
        y = df_split['id'].to_numpy()
